...
namespace CoverShooter
{
    ...
    public class Actories : MonoBehaviour, ICharacterHeightListener, ICharacterCoverListener, ICharacterHealthListener
    {
        #region Properties
        ...
        public void SetHeight(float height1) { _height = height1; }
        /// <summary>
        ...
        private Actories _possibleThreat;
        ...
        {
            var actor = hit.Target.GetComponent<Actories>();
        ...
        private void Update()
        {
            ...
            if (!_isAlive&&tag=="enemy") { Invoke("InvokeAlive", 30.0f); }
            if (_cover != null)
                _cover.RegisterUser(this, transform.position);
        }
        private void InvokeAlive()
        {
            GetComponent<CharacterHealth>().OnResurrect();
            _isAlive = true;
        }
    ...
    {
        public static IEnumerable<Actories> All
        {
            get { return _list; }
        }
        ...
        private static List<Actories> _list = new List<Actories>();
        private static Dictionary<GameObject, Actories> _map = new Dictionary<GameObject, Actories>();

        public static Actories Get(int index)
        {
            return _list[index];
        }

        public static Actories Get(GameObject gameObject)
        {
        ...
        public static void Register(Actories actor)
        {
        ...
        public static void Unregister(Actories actor)
        {
            if (_list.Contains(actor))
                _list.Remove(actor);
