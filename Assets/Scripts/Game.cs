...
public class Game : MonoBehaviour
{
    ...
    void Update()
    {
        switch (gameState)
        {
            case GameStated1.Start:
                if (Input.GetMouseButtonUp(0))
                {
                    Shuffle();
                    gameState = GameStated1.Playing;
                }
                break;
            case GameStated1.Playing:
                CheckPieceInput();
                break;
            case GameStated1.Animating:
                AnimateMovement(PieceToAnimate, Time.deltaTime);
                CheckIfAnimationEnded();
                break;
            case GameStated1.End:
                if (Input.GetMouseButtonUp(0))
                {   //reload
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if(pieceFound)
                {
                    ...
                    gameState = GameStated1.Animating;
                }
