...
public class weapon : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(walkTimer < 1f){
            fpsController.GetComponent<FirstPersonCtrll>().isFiring = true;
        }else{
            fpsController.GetComponent<FirstPersonCtrll>().isFiring = false;
        }
        ...
        if (reloadTimer < reloadTime)
        {
            ...
            fpsController.GetComponent<FirstPersonCtrll>().isReloading = true;
            return;
        }
        else if(!Input.GetButton("Fire2")){
            hasReloaded = true;
        }
        fpsController.GetComponent<FirstPersonCtrll>().isReloading = false;
        ...
        if(fireTimer >= 1 / fireRate && reloadTimer > reloadTime + 0.3f){
            // fpsController.GetComponent<FirstPersonCtrll>().IsRunning()
    ...
    void Shoot()
    {
        ...
        fpsController.GetComponent<FirstPersonCtrll>().m_MouseLook.m_CameraTargetRot *= Quaternion.Euler (-recoil, 0f, 0f);
...
//        gm.ammoInWeapons[gm.playerWeapons[gm.currentWeapon]] = currentAmmoCount;
    ...
    void DoBullet(RaycastHit hit){
            Assets.Scripts.AI.Target target = hit.transform.GetComponent<Assets.Scripts.AI.Target>();
